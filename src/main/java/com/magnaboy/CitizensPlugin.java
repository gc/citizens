...
public class CitizensPlugin extends Plugin {
    ...
    public static CitizensPlugin Instance;
...
//                .setExamine("He's on his day off."));
    ...
    //Used to initially export all citizens into their respective regions.
    private void SplitIntoRegions()
    {
        HashMap<Integer, List<Citizen>> citizenMap = new HashMap<>();
        for(Citizen c : citizens)
        {
            int regionId = c.worldLocation.getRegionID();
        ...
        {
            CitizenRegion c = new CitizenRegion(i);
            try {
                c.SaveRegion(true);
            } catch (IOException e) {
                throw new RuntimeException(e);
        ...
        for(int i : loaded)
        {
            if(!LoadedRegions.containsKey(i))
            {
                ...
                if(region != null)
                {
                    ...
                    Util.log("Loaded Region: " + i + " | Contains: " + region.Citizens.stream().count() + " citizens");
                }
            }
        }
    }
    ...
    public static void ReloadCitizens()
    {
        ...
        Instance.entityCollection.removeAll(Instance.citizens);
        Instance.despawnAll();
        Instance.citizens.clear();
        LoadedRegions.clear();
        try {
